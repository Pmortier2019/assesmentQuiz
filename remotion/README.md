...
Output per video: `out/videos/<id>.mp4` plus `out/videos/<id>.txt`, e.g.
`q-01-numerical-reasoning.mp4` + `.txt`. The id embeds the `practiceSlug`.

The `.txt` is the ready-to-paste YouTube copy: the short prompt, a
"Comment your answer" line, the correct answer + full explanation, and the
practice link with UTM tags baked in (`utm_source=youtube&utm_medium=shorts&
utm_campaign=question-shorts`, closing the issue #142 approach). It ends with a
**Tags** block — comma-separated keywords with no `#` — to paste into YouTube's
Tags field for search discovery. Paste the description + tags at upload; that is
the only manual step. (There is no pinned comment and no YouTube API: the Data
API can't pin comments anyway.)
